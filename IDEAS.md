...
- **Advanced Haptics**: Implement granular vibration feedback for Arcs/Sliders and Chains, varying intensity based on cut position and speed.
- **Dynamic Campaign Generator**: A procedural campaign mode that generates objectives based on player skill level and custom song library.
- **V3 Lighting Visualizer Editor**: A specialized UI within the Web Editor to visually sequence complex V3 lighting groups, making it easier than manual JSON editing.
- **Cross-Platform Multiplayer**: Allow WebVR, Desktop, and eventually mobile users to all compete in the same multiplayer lobbies with normalized scoring.
- **Modifier Combinations**: Special badges or leaderboards for completing songs with specific, extremely difficult modifier combinations (e.g., Ghost Notes + Pro Mode + Small Notes).
